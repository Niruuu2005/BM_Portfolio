...
- **`editor`**: teaching data only - `subjects_taught`, `study_materials`, `projects_guided`, plus `GET /admin/subjects_taught/options` and `GET /admin/me`.
...
Optional: copy `.env` in this folder or export variables in your shell. See `api/.env.example`.
...
- Public: prefix `/public` (no auth)
- Admin: prefix `/admin` (header `Authorization: Bearer <supabase access_token>`)
- `GET /admin/me` returns `{ user_id, role }` for the signed-in admin.

## Frontend

Set `VITE_API_URL` in `frontend/.env`:

- Local direct backend: `VITE_API_URL=http://localhost:8000`
