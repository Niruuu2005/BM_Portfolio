  ...
  return (
    <div>
      <div className="page-title-bar">
        <h2>Copyrights</h2>
        <button type="button" className="btn btn-primary" onClick={() => { setEdit(null); setModal(true) }}>+ Add Copyright</button>
      </div>
      <DataTable
        data={data}
        columns={COLUMNS}
      ...
      />
      <Modal isOpen={modalOpen} onClose={() => setModal(false)} title={editItem ? 'Edit Copyright' : 'Add Copyright'}>
        <CopyrightForm defaultValues={editItem} onSubmit={upsert.mutate} isLoading={upsert.isPending} />
      </Modal>
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDelId(null)} onConfirm={() => remove.mutate(deleteId)} message="Delete this copyright? This cannot be undone." />
    </div>
  )
}
