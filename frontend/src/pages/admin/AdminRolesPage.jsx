  ...
  return (
    <div>
      <div className="page-title-bar">
        ...
        <button type="button" className="btn btn-primary" onClick={() => { setEdit(null); setModal(true) }}>+ Add Role</button>
      </div>
      <DataTable
        data={data}
        columns={COLUMNS}
      ...
      />
      <Modal isOpen={modalOpen} onClose={() => setModal(false)} title={editItem ? 'Edit Role' : 'Add Role'}>
        <AdminRoleForm defaultValues={editItem} onSubmit={upsert.mutate} isLoading={upsert.isPending} />
      </Modal>
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDelId(null)} onConfirm={() => remove.mutate(deleteId)} message="Delete this role? This cannot be undone." />
    </div>
  )
}
