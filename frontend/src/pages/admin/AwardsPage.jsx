...
const COLUMNS = [
  ...
  { key: 'awarding_body', label: 'Awarded By' },
  { key: 'year', label: 'Year' },
]
  ...
  return (
    <div>
      <div className="page-title-bar">
        ...
        <button type="button" className="btn btn-primary" onClick={() => { setEdit(null); setModal(true) }}>+ Add Award</button>
      </div>
      <DataTable
        data={data}
        columns={COLUMNS}
      ...
      />
      <Modal isOpen={modalOpen} onClose={() => setModal(false)} title={editItem ? 'Edit Award' : 'Add Award'}>
        <AwardForm defaultValues={editItem} onSubmit={upsert.mutate} isLoading={upsert.isPending} />
      </Modal>
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDelId(null)} onConfirm={() => remove.mutate(deleteId)} message="Delete this award? This cannot be undone." />
    </div>
  )
}
